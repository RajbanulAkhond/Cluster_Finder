...
class TestFileIO:
    ...
    def test_generate_lattice_with_clusters(self, simple_cubic_structure, sample_cluster):
        ...
        cluster_dict = {
            ...
            "label": "X"  # Add label for DummySpecies
        }
        ...
        assert conventional_structure[0].species_string == "X0+"  # Dummy atom with zero charge
        assert isinstance(space_group_symbol, str)  # Space group symbol should be a string
        assert isinstance(point_groups, dict)  # Point groups should be a dictionary
